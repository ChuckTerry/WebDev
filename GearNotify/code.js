...
function processData(rawData) {
  ...
  if (devrimKay >= 2) pushNotification(`Devrim Key may have a 300 power engram.`);
  if (miniTool >= 2) pushNotification(`The MIDA Mini-Tool may be power 300.`);
  if (drang >= 2) pushNotification(`The Drang may be power 300.`);
  if (asherMir >= 2) pushNotification(`Asher Mir may have a 300 power engram.`);
  if (manOWar >= 2) pushNotification(`The Man O' War may be power 300.`);
  if (failsafe >= 2) pushNotification(`Failsafe may have a 300 power engram.`);
  if (sloane >= 2) pushNotification(`Sloane may have a 300 power engram.`);
  if (zavala >= 2) pushNotification(`Zavala may have a 300 power engram.`);
  if (shaxx >= 2) pushNotification(`Shaxx may have a 300 power engram.`);
  if (ikoraRey >= 2) pushNotification(`Ikora Rey may have a 300 power engram.`);
  if (banshee >= 2) pushNotification(`Banshee-44 may have a 300 power engram.`);
  if (benedict >= 2) pushNotification(`Benedict 99-40 may have a 300 power engram.`);
  if (deadOrbit >= 2) pushNotification(`Dead Orbit may have a 300 power engram.`);
  if (futureWarCult >= 2) pushNotification(`Future War Cult may have a 300 power engram.`);
  if (newMonarchy >= 2) pushNotification(`New Monarchy may have a 300 power engram.`);
